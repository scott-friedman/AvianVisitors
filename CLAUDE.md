...
- **Song signatures are a build-time Pages artifact** — the modal "song signature" bloom is a *canonical* per-species fingerprint precomputed from a clean **xeno-canto** song by `avian/scripts/build-signatures.mjs` (Node+ffmpeg) → `avian/assets/signatures.json` + bundled `avian/assets/songs/<slug>.mp3` (tap-to-play). NOT extracted from our noisy R2 clips. Re-run to expand coverage (incremental; `--force`/`--only`). The shared STFT lives in `avian/frontend/spectral-core.js` (browser + the build script both use it). Needs the xeno-canto v3 key at `~/.config/avian/xeno-canto-key` (600). **DSP rule learned 2026-06-21: compute energy/pitch from LINEAR POWER, never the dB grid** (dB summed over a linear-frequency band pins every dominant near the ceiling). Full story: `SPECTRO-CONCEPTS-PLAN.md`. Two more traps found 2026-07-29 closing the last 3 gaps: **(a) the bloom only analyses `SPEC_FLO`–`SPEC_FHI` = 1800–8500 Hz** (songbird-tuned), so a low-voiced bird is fingerprinted on its harmonics, not its fundamental — a Mallard's ~600 Hz quack renders as the 1.8–2.6 kHz rasp pinned at the floor, and `tonality()` further docks 0.25 for a dominant ≤ `SPEC_FLO+200`. That's a floor no recording can beat; accept it or add the bird to `COVERAGE_SIG_EXEMPT` — don't go hunting for a "better" clip. **(b) the q:A→q:B fallback used to be gated on `recs.length`**, so one badly-licensed A-grade hit could mask perfectly good B recordings (Cooper's Hawk: a single `by-nc-nd` A recording hid two `by-nc-sa` B ones and it reported "no compatible-license song"). Fixed in `buildOne()` — fall back whenever q:A yields no *license-compatible* candidate. If a species skips for licensing, check q:B by hand before reaching for an override.
...
- **Frame geometry is ported to the 7.3" panel** — `frame/display.py` is 800×480 7-color (`PANEL_W/H`, `INKY7`); only `frame/hardware/` CAD remains 13.3".
...
- **A Pages deploy is NOT immediately visible** (learned 2026-07-03): both `birds-origin.indianridgeroad.com` (a zone custom domain) and the ridge `/birds/` proxy edge-cache assets for **24 h**, and a Pages deploy purges neither. Symptom: a new species renders in the atlas/modal but **not the collage** (its DIMS/MASKS live inside the stale `apt.js`). Fix shipped: `avian/build-site.sh` stamps `?v=<content-hash>` onto the shell asset URLs in `index.html` (apt.js, styles.css, spectral-core.js, config.js), so changed files get fresh cache keys automatically; the HTML itself propagates in ≤5 min (origin `must-revalidate`; proxy TTL 300 s). **Extended 2026-07-06 to signatures.json + song clips** (also fetched at FIXED paths, so a new signature set was invisible for a day): build-site.sh now also content-hashes (a) the `assets/signatures.json` fetch inside apt.js, and (b) each `assets/songs/*.mp3` clip URL inside the DEPLOYED signatures.json (committed source stays bare). Hash the FINAL deployed json (after clip-versioning) so the fetch URL changes whenever the served bytes do. **Deploy gotcha that cost a redeploy 2026-07-06: don't `curl` the new hashed asset URLs through the `indianridgeroad.com` proxy during the ~1-min Pages production-alias flip** — the proxy caches whatever origin returns for 24 h, so probing a `?v=<newhash>` URL before the alias flips caches the OLD bytes at the NEW url (cache-poison; it broke one clip). **Verify via the `avianvisitors.pages.dev` ORIGIN first;** only hit the proxy once the prod alias shows the new build.
- **Mojo (`Canis volaticus`, com "Mojo") is FAKE ON PURPOSE** — the family-dog easter egg, live 2026-07-03. Art `canis-volaticus[-2].png` (flight forced on the collage via `FORCE_POSE`, perched in the modal via `MODAL_POSE`, both in `apt.js`); a Worker **cron** (`[triggers]` in `worker/wrangler.toml`) inserts "detections", and self-heals his bark clip in R2 (`master/mojo-bark.mp3` → `clips/…`); `/api/wiki` special-cases his description. **`mojoVisit()` behaviour changed 2026-07-06** (Scott's ask): he no longer sits permanently in the 1-h window. Each cron tick now gates on four things (all tunable consts in the `MOJO` object): (1) **activity** — ≥`minRecentReal` real non-Mojo detections in the last `activityWindowMin` (so he only shows when other birds are actually being heard, never on a dead hour); (2) **spacing** — ≥`minGapMin` since his own last row; (3) **daily cap** — ≤`dailyCap` per LOCAL day; (4) a `visitProb` coin-flip. Net: he surfaces a **few times a day around real activity**, then ages out. (Transition note: on switch-over day he may already be over the cap from the old cron and go quiet till the next ET day — prune old `Canis volaticus` rows if you want it visible sooner.) Do NOT "fix", dedupe, or delete him; full removal steps are in the `MOJO` block in `worker/src/index.js`.
