...
- [x] **Fish Crow** — `Corvus ossifragus` · freq 0.06  ✅ DEPLOYED LIVE 2026-07-23 — DETECTED at Sudbury (n=1) → the "add only if detected" trigger fired; sole bird in both `/api/coverage` gaps (art + signature). Keyed `--tol 34` (all-black, no pale belly to protect). QA on magenta caught an invented cream ground-blob tucked *behind the legs* (interior → keycut's border fill couldn't reach it) — flood-filled it out from a seed without touching the olive toes. NEW `species-notes.json` entry (slim glossy crow, slender bill, square/fan tail — anti-drift vs the Common Raven `corvus-corax` we also ship: no heavy bill, no shaggy throat hackles, no wedge tail). Bonus: also got a **song signature** — crows don't sing, so pinned the iconic nasal double "uh-uh" call via `signature-overrides.json` (XC815114, Richard E. Webster, tonality 0.72, CC BY-NC-SA 4.0; spectrogram = two clean ~1.8 kHz bursts). Complete species (art+bloom); coverage now 0/0.
